...
const MapComponent = () => {
  ...
  return (
    <div className='button1'>
      ...
      {/* GeoServer Layers */}
      {layersCheckedList.includes('Building layers') && <GeoServerLayer map={map} visible={true} layerName="CMCC:coimb_survey_building" />}
      {layersCheckedList.includes('Road layers') && <GeoServerLayer map={map} visible={true} layerName="CMCC:coimb_road_w68" />}
      {layersCheckedList.includes('Drone image') && <GeoServerLayer map={map} visible={true} layerName="coimb_drone_image_w68" />}
      {layersCheckedList.includes('Ward boundary') && <GeoServerLayer map={map} visible={true} layerName="coimb_ward_boundary" />}
    </div>
  );
};
